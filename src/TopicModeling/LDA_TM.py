...
#            
            ...
            df_f=pd.DataFrame({'Topic':self.topic_labels,'TopicScore':df_dist.sum().values})
        
            output_file("year_dist.html")
                    
            p = Bar(df_f, 'Topic', values='TopicScore', title="Bar Plot of Topic Scores for the year %d" %year,color='blue',legend=False)
            plots.append(p)
    ...
    def plot_doc_clustering_interia(self,max_cluster=100,min_cluster=3):
        ...
        plt.show()
#        output_file("docClustering.html")
...
#        show(p)
    ...
    def get_doc_topic_distribution_by_id(self,doc_id):
        doc_num=self.df_papers.index[self.df_papers['id']==doc_id].tolist()[0]
...
#        plt.show()
#        
        df=pd.DataFrame({'Topic':topic_labels,'TopicDistribution':df_dist.sum().values})
        
        output_file("toptpic.html")
        
        p = Bar(df, 'Topic', values='TopicDistribution', title="Bar Plot of Topic Distributions for the year %d" %year,color='green')
        if(sp):
            show(p)
    ...
    def topic_evolution_by_year(self,topic,sp=True):
        ...
        tf=pd.DataFrame({'Topic'+str(topic)+'Score':topic_score,'Year':[y for y in range(1987,2017)]})
#        ax=tf.plot(kind='bar')
...
#        
        ...
        p = Bar(tf, 'Year', values='Topic'+str(topic)+'Score', title="Bar Plot of Topic %d score for all years " %topic,color='green',legend=False)
        if(sp):
            show(p)
...
#                    plt.show()
#                    
                    df_f=pd.DataFrame({'Topic':self.topic_labels,'TopicScore':df[y].values})
        
                    output_file("authorscorebyyear.html")
                    
                    p = Bar(df_f, 'Topic', values='TopicScore', title="Bar Plot of Topic Distributions for the year %d" %y,color='blue',legend=False)
                    plots.append([p])
...
#            plt.show()
            ...
            pl = Bar(df_f, 'Years', values='TopTopic', title="Bar Plot of top Topic",color='blue',legend=False)
            show(pl)
